...
def compute_sw_time_only_from_df(
    ...
    if df.empty:
        raise ValueError(
            "vehicles_table is empty. Make sure you ran cell F1 (run_experiment) in this "
            "session and that excel_path points to the freshly generated file."
        )

    # --- required identifiers ---
    # COL_N may be absent in Excel files generated before N was added; fall back to 0.
    if COL_N not in df.columns:
        df[COL_N] = 0

    missing = [c for c in (COL_STRATEGY, COL_SERVED, COL_RUN) if c not in df.columns]
    if missing:
        raise ValueError(
            f"vehicles_table is missing required columns: {missing}. "
            "The Excel file was likely generated by an older version of the code — "
            "please re-run cell F1 to regenerate results."
        )
...
def plot_demand_entry_histogram(
    veh_df: pd.DataFrame,
    *,
    ...
    """Histogram of vehicle entry times showing temporal demand profile.

    Uses the ``entry_time`` column (actual slot vehicle entered the network).
    Falls back to ``desired_entry`` if ``entry_time`` is absent / all-NaN.

    Args:
        veh_df: vehicles_table DataFrame (from demand run or full batch run).
        peak_slot: if given, draws a vertical dashed line marking the peak.
        bins: number of histogram bins; defaults to the number of distinct slots.
        fs: base font size.
    """
    df = veh_df.copy()
    ...
    if time_col not in df.columns or df[time_col].isna().all():
        time_col = "desired_entry"
    if time_col not in df.columns or df[time_col].isna().all():
        print("plot_demand_entry_histogram: no entry_time or desired_entry column, skipping.")
        return

    times = pd.to_numeric(df[time_col], errors="coerce").dropna()
    if times.empty:
        print("plot_demand_entry_histogram: no numeric entry times, skipping.")
        return

    n_bins = bins or max(10, int(times.max() - times.min() + 1))

    fig, ax = plt.subplots(figsize=(10, 4))
    ax.hist(times, bins=n_bins, color="steelblue", edgecolor="white", linewidth=0.4)

    if peak_slot is not None:
        ax.axvline(peak_slot, color="crimson", linestyle="--", linewidth=1.8, label=f"peak_slot={peak_slot}")
        ax.legend(fontsize=fs - 1)

    ax.set_xlabel("Time slot", fontsize=fs)
    ax.set_ylabel("Number of vehicles", fontsize=fs)
    ax.set_title("Temporal demand profile — vehicle entry distribution", fontsize=fs + 1)
...
def plot_demand_over_time(
    ts_df: pd.DataFrame,
    *,
    metric: str = "alloc_count",
    peak_slot: int | None = None,
    top_n_edges: int | None = None,
    fs: int = 13,
) -> None:
    """Two-panel demand-over-time view from edge_timeslices.

    Top panel — total ``metric`` summed across all edges per time slot:
        shows the global demand curve and how sharp the peak is.

    Bottom panel — stacked area of the top-N busiest edges over time:
        reveals *where* demand concentrates spatially near the peak.

    Args:
        ts_df: edge_timeslices DataFrame (columns: t, edge, alloc_count, util, …).
        metric: column to aggregate; ``"alloc_count"`` or ``"util"``.
        peak_slot: if given, draws a vertical dashed line on both panels.
        top_n_edges: number of busiest edges to show in the stacked panel (default 10).
        fs: base font size.
    """
    if ts_df.empty:
        print("plot_demand_over_time: edge_timeslices is empty, skipping.")
        return

    needed = {COL_T, metric, "edge"}
    missing = needed - set(ts_df.columns)
    if missing:
        print(f"plot_demand_over_time: missing columns {missing}, skipping.")
        return
    ...
    # --- panel 1: total demand per slot ---
    by_t = df.groupby(COL_T)[metric].sum().sort_index()

    # --- panel 2: top-N edges stacked area ---
    edge_totals = df.groupby("edge")[metric].sum().nlargest(top_n)
    top_edges = edge_totals.index.tolist()
    sub = df[df["edge"].isin(top_edges)]
    pivot = (
        sub.pivot_table(index=COL_T, columns="edge", values=metric, aggfunc="sum")
        .reindex(sorted(df[COL_T].unique()))
        .fillna(0)
    )
    pivot.columns = [str(e)[:25] for e in pivot.columns]

    _, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 8), sharex=True)

    ax1.bar(by_t.index, by_t.values, color="steelblue", width=0.8, alpha=0.85)
    ax1.set_ylabel(metric, fontsize=fs)
    ax1.set_title(f"Total {metric} across all edges per time slot", fontsize=fs + 1)
    ax1.grid(axis="y", alpha=0.3)

    ax2.stackplot(pivot.index, pivot.T.values, labels=pivot.columns, alpha=0.8)
    ax2.set_ylabel(metric, fontsize=fs)
    ax2.set_xlabel("Time slot", fontsize=fs)
    ax2.set_title(f"Top-{top_n} busiest edges — {metric} over time", fontsize=fs + 1)
    ax2.legend(loc="upper right", fontsize=max(6, fs - 4), ncol=2)
    ax2.grid(axis="y", alpha=0.3)

    if peak_slot is not None:
        for ax in (ax1, ax2):
            ax.axvline(peak_slot, color="crimson", linestyle="--", linewidth=1.6,
                       label=f"peak_slot={peak_slot}")
